...
parser = argparse.ArgumentParser(description='Rota dos coordenadas, útil para verificar validez de datos y conversion entre cantidad de bits.')
...
Xr = X0r*math.cos(angle*math.pi/180) - Y0r*math.sin(angle*math.pi/180)
Yr = X0r*math.sin(angle*math.pi/180) + Y0r*math.cos(angle*math.pi/180)

print(f"X0r: {X0r} (X0c: {X0c}, {hex(int(bindigits(X0c, COORDINATES_WIDTH), 2))}, {bindigits(X0c, COORDINATES_WIDTH)}), Y0r: {Y0r} (Y0c: {Y0c}, {hex(int(bindigits(Y0c, COORDINATES_WIDTH), 2))}, {bindigits(Y0c, COORDINATES_WIDTH)}), Angle: {angle} ({hex(int(bindigits(angle, ANGLE_INTEGER_WIDTH), 2))}, {bindigits(angle, ANGLE_INTEGER_WIDTH)})")
print(f"Xesc: {Xesc} (Xc: {Xc} {hex(int(bindigits(Xc, COORDINATES_WIDTH), 2))}, {bindigits(Xc, COORDINATES_WIDTH)}), Yesc: {Yesc} (Yc: {Yc} {hex(int(bindigits(Yc, COORDINATES_WIDTH), 2))}, {bindigits(Yc, COORDINATES_WIDTH)})")
